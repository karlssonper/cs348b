...
#include "shapes/naiadparticle.h"
...
Reference<Shape> MakeShape(const string &name,
    ...
    else if (name == "naiadmesh")
            s = CreateNaiadMeshShape(object2world, world2object, reverseOrientation,
                                        paramSet, &graphicsState.floatTextures);
    else if (name == "naiadparticle")
                s = CreateNaiadFoamParticles(object2world, world2object, reverseOrientation,
                                            paramSet);
    else if (name == "naiadfoam")
                s = CreateNaiadFoam(object2world, world2object, reverseOrientation,
                                            paramSet);
    else if (name == "heightfield") {
    //    s = CreateHeightfieldShape(object2world, world2object, reverseOrientation,
    //                               paramSet);
    //else if (name == "perheightfield") {
#define T_CELLS_PER_GRID_AXIS 2
        int nu = paramSet.FindOneInt("nu", -1);
        int order = ceil(log(nu)/log(T_CELLS_PER_GRID_AXIS)) - 1;
        printf("Order: %i\n", order);
        switch (order) {
#define CASE_GRID(x)                                                   \
        case x:                                                        \
        s = CreatePerHeightfieldShape<T_CELLS_PER_GRID_AXIS,x>(        \
            object2world, world2object, reverseOrientation, paramSet); \
            break;                                                     \
