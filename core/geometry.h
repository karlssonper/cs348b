...
public:
    // Ray Public Methods
    Ray() : hitFoam(false),mint(0.f), maxt(INFINITY), time(0.f), depth(0),  zbuffer(false), zbuffer_depth(0.f) { }
    Ray(const Point &origin, const Vector &direction,
        float start, float end = INFINITY, float t = 0.f, int d = 0)
        : hitFoam(false),o(origin), d(direction), mint(start), maxt(end), time(t), depth(d), zbuffer(false), zbuffer_depth(0.f) { }
    Ray(const Point &origin, const Vector &direction, const Ray &parent,
        float start, float end = INFINITY)
        : hitFoam(false),o(origin), d(direction), mint(start), maxt(end),
          time(parent.time), depth(parent.depth+1), zbuffer(false), zbuffer_depth(0.f) { }
    ...
    // Ray Public Data
    mutable bool hitFoam;
    mutable float alphaFoam;
    Point o;
    Vector d;
