...
Spectrum SpecularReflect(const RayDifferential &ray, BSDF *bsdf,
    ...
    if (pdf > 0.f && !f.IsBlack() && AbsDot(wi, n) != 0.f) {
        ...
        rd.hitFoam = ray.hitFoam;
        bool foamYet = rd.hitFoam;
        ...
        if (!foamYet && rd.hitFoam && rd.alphaFoam) {
            rd.alphaFoam *= 0.7;
            float rgb[3];
            L.ToRGB(rgb);
            rgb[0] = 1.0f - (1 - rgb[0]) * (1-rd.alphaFoam);
            rgb[1] = 1.0f - (1 - rgb[1]) * (1-rd.alphaFoam);
            rgb[2] = 1.0f - (1 - rgb[2]) * (1-rd.alphaFoam);
            L = L.FromRGB(rgb);
        }
